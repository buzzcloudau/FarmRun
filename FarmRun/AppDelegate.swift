...
class AppDelegate: UIResponder, UIApplicationDelegate {
	...
	func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
		...
		GAI.sharedInstance().trackerWithTrackingId("UA-000000000-0")
		GAI.sharedInstance().defaultTracker.allowIDFACollection = true
		GAI.sharedInstance().trackUncaughtExceptions = true
		GAI.sharedInstance().dispatchInterval = 20
		//println("GAILogLevel.Verbose \(GAI.sharedInstance().logger.logLevel)")
		GAI.sharedInstance().logger.logLevel = GAILogLevel.Error

		let build = GAIDictionaryBuilder.createEventWithCategory("ui_action", action: "app_launched",label:"launch",value:nil).build();
...
//			var alert : UIAlertView = UIAlertView(title: "storeDidChange", message: "Notification: \(notification)", delegate: nil, cancelButtonTitle: "Ok")
//			alert.show()
